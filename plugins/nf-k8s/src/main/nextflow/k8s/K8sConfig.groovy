...
class K8sConfig implements ConfigScope {
    ...
    K8sConfig(Map opts) {
        ...
        namespace = opts.namespace ?: namespaceFromEnv()
        ...
        serviceAccount = opts.serviceAccount ?: 'default'
        storageClaimName = opts.storageClaimName ?: storageClaimNameFromEnv()
        storageMountPath = opts.storageMountPath ?: '/nf-workdir'
        storageSubPath = opts.storageSubPath
        userName = opts.userName

        final userDir = opts.userDir
        if( userDir )
            log.warn "K8s `userDir` has been deprecated -- Use `launchDir` instead"
        launchDir = userDir ?: opts.launchDir ?: "${storageMountPath}/${getUserName()}"
    ...
    private static String defaultContext() {
        'arn:aws:eks:us-west-2:812206152185:cluster/prion-dev'
    }

    private static String namespaceFromEnv() {
        final wsId = System.getenv('FLYTE_INTERNAL_EXECUTION_PROJECT')
        if( wsId == null )
            throw new RuntimeException("failed to get workspaceId")
        return "${wsId}-development"
    }

    private static String storageClaimNameFromEnv() {
        final name = System.getenv('K8S_STORAGE_CLAIM_NAME')
        if( name == null )
            throw new RuntimeException("failed to get storage claim name from K8S_STORAGE_CLAIM_NAME")
        return name
    }

    boolean getCleanup(boolean defValue=true) {
        cleanup == null ? false : cleanup
    }
