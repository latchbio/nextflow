...
class TaskProcessor {
    ...
    final protected void invokeTask( Object[] args ) {
        ...
        // the remote task must be created after the task context is setup
    ...
    void writeToStoreDir(TaskRun task, Path source, LatchPath storeDir) {
        final rel = task.workDir.relativize(source)
        final target = storeDir.resolve(rel)
    ...
    void storeOutputs(TaskRun task) {
        ...
        final storeDir = task.config.getStoreDir()
        if( storeDir == null || storeDir.scheme != 'latch' )
            return

        final lp = (LatchPath)storeDir
        lp.createDirIfNotExists()

        for( Path output : getPublishFiles(task) )
            writeToStoreDir(task, output, lp)
    }
    ...
    private void uploadTaskLogs( TaskRun task ) {
        ...
        final logDir = System.getenv('LATCH_LOG_DIR')
        if( logDir == null )
            return

        final targetDir = FileHelper.asPath(logDir)
        if( targetDir.scheme != 'latch' ) {
        ...
        for( String name : [
            ...
            try {
                final source = task.workDir.resolve(name)
                final subPath = session.workDir.relativize(source)
                final target = targetDir.resolve('work').resolve(subPath)
